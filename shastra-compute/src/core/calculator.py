...
from src.core.models.chart import (
    CanonicalChart,
    PlanetPosition,
    HouseCusp,
    Aspect,
...
NAKSHATRA_SPAN = 13.0 + 20.0 / 60.0  # 13 deg 20' = 13.33333... deg
PADA_SPAN = NAKSHATRA_SPAN / 4.0      # 3 deg 20' = 3.33333... deg
...
def _sign_index(longitude: float) -> int:
    """Return the zodiac sign index (0-11) for a given ecliptic longitude."""
    return int(longitude // 30) % 12


def _sign_name(longitude: float) -> str:
    """Return the zodiac sign name for a given ecliptic longitude."""
    return SIGNS[_sign_index(longitude)]


def _sign_degree(longitude: float) -> float:
    """Return the degree within the sign (0-30) for a given ecliptic longitude."""
...
class ChartCalculator:
    ...
    def __init__(self, ephe_path: Optional[str] = None):
        """Initialize the calculator, optionally pointing to ephemeris files."""
        if ephe_path:
    ...
    def compute_chart(
        ...
        """Compute a full natal chart for the given birth data.
        ...
        date_of_birth : date
            Date of birth.
        time_of_birth : time | None
            Time of birth (noon is used when unknown).
        latitude, longitude : float
            Geographic coordinates of the birthplace.
        timezone_str : str
            IANA timezone string (e.g. ``Asia/Kolkata``).
        birth_time_quality : str
            One of ``exact``, ``approximate``, ``unknown``.
        birth_profile_id : str
            Optional external identifier for the birth profile.

        Returns
        -------
        CanonicalChart
            The fully computed chart with planets, houses, aspects, and dasha.
        """
        ...
        # Ketu = Rahu + 180 deg (shadow point, always retrograde like Rahu)
    ...
    def compute_transits(self, transit_date: date, natal_chart: CanonicalChart) -> dict:
        ...
        ----------
        transit_date : date
            The date for which to compute transits.
        natal_chart : CanonicalChart
            The natal chart to compare transits against.

        Returns
        -------
        dict
            Contains ``transit_planets`` and ``transit_to_natal_aspects``.
        """
    ...
    def _resolve_local_datetime(
        ...
        """Resolve the local datetime from birth data, defaulting to noon if unknown."""
    ...
    def _initial_confidence(quality: BirthTimeQuality) -> dict:
        """Return a confidence metadata dict based on birth-time quality."""
    ...
    def _assign_houses_placidus_tropical(
        ...
        """Assign tropical planets to houses using tropical Placidus cusps."""
    ...
    def _compute_aspects(raw_positions: list[dict]) -> list[Aspect]:
        """Compute all natal aspects between planets."""
        aspects: list[Aspect] = []
        n = len(raw_positions)
        for i in range(n):
    ...
    def _compute_vimshottari(
        ...
        """Compute the maha and antar dasha at birth based on Moon's sidereal longitude."""
